/// \mainpage
/// librf: A Fast C++ Implementation of Random Forests.
#pragma once
#ifndef LIBRF_DECISION_FOREST_HPP
#define LIBRF_DECISION_FOREST_HPP
...
namespace librf
{
    ...
    public:
        ...
        size_t learn(const libnd::View<Feature> &, const libnd::View<Label> &,
    ...
    private:
        struct ComparisonByFeature
        {
            ...
            ComparisonByFeature(
                const libnd::View<Feature> &features,
            ...
            const libnd::View<Feature> &features_;
    ...
    public:
        ...
        DecisionTree();
        void learn(const libnd::View<Feature> &, const libnd::View<Label> &,
                   std::vector<size_t> &);
        template <class RandomEngine>
        void learn(const libnd::View<Feature> &, const libnd::View<Label> &,
                   std::vector<size_t> &, RandomEngine &);
        ...
        void predict(const libnd::View<Feature> &, std::vector<Label> &) const;
    ...
    public:
        ...
        void predict(const libnd::View<Feature> &, libnd::MDArray<Probability> &) const;
        void serialize(std::ostream &) const;
    ...
    ///
    template <class FEATURE, class LABEL>
    inline DecisionNode<FEATURE, LABEL>::DecisionNode()
        : featureIndex_(),
          threshold_(),
          label_(),
          isLeaf_(false)
    ...
    */
        ...
        for (size_t j = 0; j < numberOfFeaturesToBeAssessed; ++j)
        {
            ...
            for (;;)
            {
                ...
                while (thresholdIndex + 1 < sampleIndexEnd && features(sampleIndices[thresholdIndex], fi) == features(sampleIndices[thresholdIndex + 1], fi))
                {
                ...
                for (size_t s = 0; s < 2; ++s)
                {
                    ...
                    {
                        giniCoefficients[s] =
                            static_cast<double>(numbersOfDistinctPairs[s]) / (numbersOfElements[s] * (numbersOfElements[s] - 1));
                    }
                }
...
#pragma omp critical
    ...
    ///
    template <class FEATURE, class LABEL>
    inline DecisionTree<FEATURE, LABEL>::DecisionTree()
        : decisionNodes_()
    {
    }
        ...
        RandomEngine &randomEngine)
    {
        ...
        // learn root node
        {
            ...
            size_t thresholdIndex = decisionNodes_.back().learn(
                ...
                randomEngine);
            if (!decisionNodes_[0].isLeaf())
            { // if root node is not pure
                queue.push(
                    TreeConstructionQueueEntry(
                        0,                       // node index
        ...
        while (!queue.empty())
        {
            ...
            {
                assert(
                    thresholdIndexNew >= sampleIndexBegin && thresholdIndexNew < thresholdIndex);
            }
#endif
            ...
            {
                assert(
                    thresholdIndexNew >= thresholdIndex && thresholdIndexNew < sampleIndexEnd);
            }
#endif
    ...
    DecisionTree<FEATURE, LABEL>::decisionNode(
        const size_t decisionNodeIndex) const
    {
    ...
    inline DecisionForest<FEATURE, LABEL, PROBABILITY>::DecisionForest()
        : decisionTrees_()
    {
    }
...
#pragma omp parallel for schedule(guided)
    ...
    DecisionForest<FEATURE, LABEL, PROBABILITY>::predict(
        const libnd::View<Feature> &features,
        libnd::MDArray<Probability> &labelProbabilities) const
    {
        if (size() == 0)
        {
...
#endif // #ifndef LIBRF_DECISION_FOREST_HPP